...
Copilotはエディター内のアシスタントで、Ame Copilotを使用してワークフローの構築や編集を支援し、それらを理解して改善するのに役立ちます。以下のことができます：

- **説明**: Ameや現在のワークフローに関する質問に回答
- **ガイド**: 編集の提案やベストプラクティスの提示
- **編集**: 承認後にブロック、接続、設定の変更を実行

<Callout type="info">
  CopilotはAmeが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（ame.ai → 設定 → Copilot）でCopilot APIキーを生成してください
  1. [ame.ai](https://sim.ai) → 設定 → Copilotに移動し、Copilot APIキーを生成
