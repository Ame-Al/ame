...
Ame calcule automatiquement les coûts pour toutes les exécutions de flux de travail, offrant une tarification transparente basée sur l'utilisation des modèles d'IA et les frais d'exécution. Comprendre ces coûts vous aide à optimiser les flux de travail et à gérer efficacement votre budget.
...
<Tabs items={['Modèles hébergés', 'Apportez votre propre clé API']}>
  <Tab>
    **Modèles hébergés** - Ame fournit des clés API avec un multiplicateur de prix de 1,4x pour les blocs Agent :
...
Lorsque les workflows utilisent des blocs d'outils avec les clés API hébergées par Ame, les coûts sont facturés par opération. Utilisez vos propres clés via BYOK pour payer directement les fournisseurs.
...
Ame utilise un modèle de facturation **abonnement de base + dépassement** :
...
Lorsque le dépassement non facturé atteint 50 $, Ame facture automatiquement le montant total non facturé.
