...
Copilot ist dein Assistent im Editor, der dir hilft, Workflows mit Ame Copilot zu erstellen und zu bearbeiten sowie diese zu verstehen und zu verbessern. Er kann:

- **Erklären**: Beantwortet Fragen zu Ame und deinem aktuellen Workflow
...
<Callout type="info">
  Copilot ist ein von Ame verwalteter Dienst. Für selbst gehostete Installationen generiere einen Copilot API-Schlüssel in der gehosteten App (ame.ai → Einstellungen → Copilot)
  1. Gehe zu [ame.ai](https://sim.ai) → Einstellungen → Copilot und generiere einen Copilot API-Schlüssel
...
- **Dokumentation**: Verweise auf Ame-Dokumentation
