...
The CLI authenticates with a Ame API key. `sim login` mints and stores one; in CI
...
Create the key in Ame under **Settings → API keys**. Store it as a secret in your
