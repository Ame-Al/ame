---
title: Ame Mailer
description: Send emails to your workspace and let Ame handle them as tasks.
---

import { Callout } from 'fumadocs-ui/components/callout'
import { FAQ } from '@/components/ui/faq'

Ame Mailer gives your workspace a dedicated email address. Forward or send emails to it and Ame will process them as tasks — reading the subject, body, and any attachments, then replying to the thread with the result.
...
**Attachments are fully supported.** Images, PDFs, and documents (up to 10 MB each) are read by Ame and displayed inline in the conversation; images show as previews.

| Good email | Why it works |
|------------|-------------|
| "Summarize the attached PDF and list action items" | Clear task with an attachment |
| "What's in this image?" with a photo attached | Ame reads and describes the image |
...
External senders are email addresses that can create inbox tasks. They are not the same as external workspace members, who have workspace access in Ame without joining your organization.
...
| **Processing** | Ame is actively working on it |
...
<FAQ items={[
  { question: "Can I use my own email domain?", answer: "Not currently. All inbox addresses use the @mothership.ame.ai domain. You can customize the prefix (e.g., acme@mothership.ame.ai) but not the domain itself." },
  ...
  { question: "Can I reply to Ame's email responses?", answer: "Yes. Replies in the same email thread are linked to the original conversation, so you can continue the interaction from your email client." },
