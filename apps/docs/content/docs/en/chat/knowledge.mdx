...
Describe the knowledge base and Ame creates it:
...
Add documents by attaching files to your message, pasting text, or pointing Ame at a URL:
...
Ame processes and indexes each document automatically. Once indexed, the content is searchable by any Agent block that has the knowledge base attached.

{/* TODO: Screenshot of Ame confirming a document was added and indexed — showing the document name and its indexed status in the knowledge base. */}

## Querying Knowledge Bases

Ask Ame a question and it searches the specified knowledge base to answer:
...
Connectors are configured through the knowledge base settings, not through Chat. Once connected, all synced content is immediately searchable by Ame and by any Agent block with the knowledge base attached.

Ame ships with 49 built-in connectors, including Notion, Google Drive, Slack, GitHub, Confluence, HubSpot, Salesforce, Gmail, and more.
...
<FAQ items={[
  ...
  { question: "How do I keep a knowledge base up to date?", answer: "Set up a connector to sync from an external source automatically — see Connectors above. For one-off updates, ask Ame to add or replace the document directly." },
]} />
