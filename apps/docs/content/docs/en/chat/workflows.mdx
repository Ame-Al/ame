...
Describe a workflow and Ame builds it. Reference an existing one by name and Ame edits it. No canvas navigation required — every change appears in the resource panel in real time.

## Creating Workflows

Describe what the workflow should do — what triggers it, what it should do, which integrations it needs, and what it should return. Ame builds it and opens the canvas in the resource panel.
...
Open an existing workflow with `@workflow-name` or the **+** menu, then describe the change. Ame reads the current structure before modifying it — you don't need to explain what already exists.
...
Ask Ame to run a workflow and it handles the execution:
...
Ame can retrieve and interpret execution logs for any workflow in the workspace:
...
Ame can also roll back: "Revert the billing workflow to the version from last Tuesday."
...
Ame can create and manage folders to keep your workspace organized.
...
{/* TODO: Screenshot showing Ame confirming a folder or workflow organization action — e.g., a message confirming "Moved 'invoice-processor' into 'billing' folder" with the resource panel showing the folder open. */}

## Workflow Variables

Ame can set global variables on a workflow — values accessible across all blocks in that workflow at runtime:
...
<FAQ items={[
  { question: "Can Ame edit a workflow while it's deployed?", answer: "Yes. Editing a workflow does not affect the live deployment. The deployed version is a snapshot — you need to ask Ame to redeploy to push changes to production." },
  { question: "Can I run a workflow with specific inputs from Chat?", answer: "Yes. Describe the inputs in your message and Ame passes them to the workflow's start block." },
  { question: "How does Ame know what my workflow does?", answer: "When you reference a workflow, Ame loads its full structure — every block, connection, and configuration — before acting on it." },
