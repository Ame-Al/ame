...
Ame can act on your behalf right now — send a message, create an issue, call an API — or on a schedule, running a prompt automatically every hour, day, or week. It can also connect integrations, set environment variables, add MCP servers, and create custom tools.

## Scheduled Jobs

A scheduled job is a saved Chat prompt that runs on a cron schedule. On each run, Ame reads the current workspace state and executes the job's prompt as if you had just sent it.
...
Ame sets the cron expression and stores the job prompt. The first run happens at the next scheduled time.
...
For requests that should happen right now — without building a workflow — just ask. Ame acts immediately using the credentials connected to your workspace.
...
If an integration isn't connected, Ame walks you through connecting it.

## Connecting Integrations

Ame can connect new OAuth integrations and API credentials on demand:

- "Connect my Google account"
- "Add the Slack workspace for our team"
- "Set up GitHub with my personal access token"

{/* TODO: Screenshot of Ame walking through connecting an integration — e.g., the Integration subagent active with an OAuth prompt or confirmation that a credential was connected. */}

Once connected, credentials are available to Ame for direct actions and scheduled jobs, and to all workflows in the workspace.
...
{/* TODO: Screenshot of Ame confirming an environment variable was set — e.g., a response message showing the variable name was saved. */}
...
Ame can add and manage MCP servers connected to your workspace:
...
{/* TODO: Screenshot of Ame confirming an MCP server was added or updated — showing the server name and its status. */}

## Custom Tools

Custom tools are single HTTP endpoints you define manually — useful for internal APIs and services that don't have a built-in Ame integration or an MCP server. Once created, they appear in the workflow editor alongside built-in tools and can be called from any Agent block.

Ame can build custom tools from a description:
...
<FAQ items={[
  { question: "What's the difference between a scheduled job and a deployed workflow?", answer: "A scheduled job runs a Chat prompt on a cron schedule — Ame decides what to do each time based on current workspace state. A deployed workflow runs a fixed, deterministic graph of blocks. Use jobs when you want Ame to reason and adapt; use workflows when you want predictable, auditable execution." },
  ...
  { question: "How do I know what integrations are connected?", answer: "Ask Ame: 'What integrations are connected to this workspace?' or check the Integrations page." },
  { question: "Can direct actions be undone?", answer: "No. Actions like sending emails, posting to Slack, or creating records are immediate and irreversible. Confirm the details before asking Ame to act." },
