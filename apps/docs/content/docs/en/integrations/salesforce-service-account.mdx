...
description: Set up a Salesforce External Client App with the Client Credentials Flow or the JWT Bearer Flow and a dedicated integration user to use Salesforce in Ame workflows
---
...
Ame supports both server-to-server flows. Pick one:

| | **Client credentials** | **JWT bearer** |
|---|---|---|
| What Ame stores | Consumer key + **consumer secret** | Consumer key + **private key** |
| Who calls run as | The app's **Run As** user | The **username** you name in Ame |
...
You need a Salesforce **admin** to create the External Client App and the integration user. Every API call Ame makes runs with the integration user's permissions, so plan that user's profile and permission sets deliberately.
...
<Steps>
  ...
  <Step>
    Add the OAuth scopes **Manage user data via APIs (api)** and **Access unique user identifiers (openid)** — `api` is required for the flow, and `openid` lets Ame look up the integration user's name via the userinfo endpoint (the instance URL comes back in the token response itself)
  </Step>
  <Step>
...
Every API call Ame makes executes with this user's permissions.
...
<Callout type="warn">
The Consumer Secret plus the Run As configuration is full API access as the integration user. Treat both values like passwords — do not commit them to source control or share them publicly. Ame encrypts them at rest.
</Callout>
...
If you already have a classic Connected App, it keeps working and the credential fields are identical — the token endpoint and Ame configuration don't change. Configure it the classic way:
...
The JWT Bearer Flow authenticates with an uploaded certificate rather than a shared secret, and runs as a username you name in Ame rather than the app's Run As user. Set this up **instead of** steps 3 and 4 above. Steps 1, 2, and 5 are the same — but in step 2, skip the Client Credentials Flow substep; JWT bearer does not use that flow.

<Steps>
  <Step>
    Generate a key pair. `server.key` stays with Ame; `server.crt` is uploaded to Salesforce:
    ...
    Keep `server.key` somewhere safe — Ame stores it encrypted and never shows it again.
...
Ame also accepts other partitioned My Domain hosts (`scratch`, `demo`, `patch`, `trailblaze`, `free`).
...
A permissions gap surfaces at run time as a Salesforce API error; fix it on the integration user's permission sets — no changes are needed in Ame.
...
## Adding the Service Account to Ame
...
<Steps>
  ...
  <Step>
    Search for "Salesforce" and open it, then click **Add to Ame** and choose **Add integration user app**
  ...
  <Step>
    Click **Add integration user app**. Ame verifies the credentials by minting a real access token against your My Domain host. A host that doesn't resolve gets its own error message; every other misconfiguration surfaces as a general authentication error — re-check the values and the app's OAuth policies.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "JWT bearer fails with app_not_found — what now?", answer: "Salesforce could not find the app in the org the audience names. Check that the My Domain host belongs to the same org the app is installed in, and that the consumer key is that app's. Since Spring '26 the audience must be your My Domain URL — Ame always uses it, which is why the host has to be exactly right." },
  { question: "JWT bearer fails with an invalid assertion or signature — what now?", answer: "The certificate uploaded to Salesforce does not match the private key pasted into Ame. Regenerate the pair together, re-upload server.crt to the app, and re-paste server.key. Also confirm you pasted the key and not the certificate — Ame flags that inline." },
  { question: "My private key is passphrase-protected — will it work?", answer: "No. Ame collects no passphrase, so decrypt the key first: openssl rsa -in server.pass.key -out server.key. Both PKCS#1 (\"BEGIN RSA PRIVATE KEY\") and PKCS#8 (\"BEGIN PRIVATE KEY\") containers are accepted, so whichever your OpenSSL version emits is fine." },
  ...
  { question: "Adding the credential fails with an authentication error even though the key and secret are right — what now?", answer: "Salesforce likely rejected the token request with invalid_grant (e.g. 'no client credentials user enabled'), which Ame reports as not being able to authenticate with those credentials. The app either doesn't have Enable Client Credentials Flow checked, or no Run As user is set. Edit the app's OAuth policies (External Client App Manager → your app → Policies, or a legacy Connected App's Manage → Edit Policies), enable the flow, and set Run As to your integration user." },
  ...
  { question: "How do I rotate the credentials?", answer: "For client credentials, rotate the Consumer Secret from the app's OAuth settings (External Client App: Settings → OAuth Settings → Consumer Key and Secret; legacy Connected App: Manage Consumer Details), then update the credential in Ame with the new secret. For JWT bearer, generate a new key pair, upload the new certificate to the app, and paste the new private key into Ame. Either way, reconnecting asks you to restate the authentication method — Ame never returns a stored secret, so it cannot pre-select it for you." },
  { question: "What happens if the integration user is deactivated or frozen?", answer: "All token minting fails with invalid_grant and every workflow using the credential stops. Reactivate the user (or point Run As at a new integration user) and runs resume — no changes needed in Ame." },
]} />
