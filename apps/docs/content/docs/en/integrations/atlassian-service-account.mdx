...
description: Set up an Atlassian service account with a scoped API token to use Jira, Jira Service Management, and Confluence in Ame workflows
---
...
<Steps>
  ...
  <Step>
    Choose **API token** as the authentication type (not OAuth 2.0 — Ame uses the API token flow)
  ...
  <Step>
    Select the scopes the token needs. The minimum set Ame's Jira and Confluence blocks expect is:
    ...
    **Confluence (granular):**
    ...
    Add more scopes only if you need the corresponding operations (delete, manage webhooks, etc.). The full list of scopes Ame's blocks may use is documented in [Atlassian's developer reference](https://developer.atlassian.com/cloud/jira/platform/scopes-for-oauth-2-3LO-and-forge-apps/).
...
<Callout type="warn">
The API token is bearer credentials for the service account. Treat it like a password — do not commit it to source control or share it publicly. Ame encrypts the token at rest.
</Callout>
...
<Steps>
  ...
  <Step>
    Click **Add to Ame** and choose **Add service account**

    {/* TODO(screenshot): Jira integration page with the "Add to Ame" dropdown open */}
  </Step>
  <Step>
    ...
    <div className="flex justify-center">
      <Image
        src="/static/credentials/atlassian/ame-add-modal.png"
  ...
  <Step>
    Click **Add service account**. Ame verifies the token by calling Atlassian's `/myself` endpoint through the gateway — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<div className="flex justify-center">
  <Image
    src="/static/credentials/atlassian/ame-jira-block-credential.png"
...
<FAQ items={[
  ...
  { question: "How do I rotate the API token?", answer: "Create a new token from the same service account in admin.atlassian.com, update the credential in Ame with the new token, and once it's working, revoke the old one." },
