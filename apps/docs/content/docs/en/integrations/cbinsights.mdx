...
**Using CB Insights in Ame**

This integration covers every non-streaming v2 endpoint. Authentication is a client-credential exchange — Ame trades your client ID and secret for a short-lived bearer token on your behalf and refreshes it automatically, so there is no token to manage in the workflow.

**Key benefits of using CB Insights in Ame:**
