...
[Snowflake](https://www.snowflake.com/) is a cloud data platform that separates storage from compute: data lives in databases and schemas, while virtual warehouses supply the compute that runs each query. Ame connects to it over the Snowflake SQL API using a programmatic access token.

With the Snowflake integration in Ame, you can:
...
| `maxRows` | number | No | Maximum result rows; defaults to 1000 with a Ame safety limit of 10000 |
