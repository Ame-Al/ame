...
With the Microsoft SQL Server integration in Ame, you can:
...
In Ame, the Microsoft SQL Server integration allows your agents to read from and write to a SQL Server database as part of a workflow — pulling records to feed downstream logic, writing agent output back to a table, and inspecting the schema before generating a query. Connections take a host, port, database, credentials, and TLS settings, so the same block works against a self-hosted instance or a managed one.
{/* MANUAL-CONTENT-END */}
