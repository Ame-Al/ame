...
Note that **Synthetic monitors are the one part of this integration on Environment API v1** — Ame handles the path difference, but the token scopes differ from the v2 endpoints.
...
Every selector field in the block has a wand — describe what you want in plain language and Ame writes the selector for you.

**Settings objects**

Most Dynatrace configuration is a *settings object*: a JSON `value` whose shape is defined by a *schema* such as `builtin:alerting.maintenance-window`. There is no fixed structure Ame can validate for you, so the reliable way to write one is to read an existing object of the same schema first and mirror its `value`. Get Settings Object also returns an `updateToken` — pass it back on update or delete and the call fails rather than overwriting a change someone else made in the meantime.

**Pagination**

List operations return a `nextPageKey` (a `nextSliceKey` for log search). Feed it back into the next call to read the following page. Dynatrace encodes the original filters into the cursor, so Ame sends the cursor alone and ignores the other filters on that call — which is what the API requires.
{/* MANUAL-CONTENT-END */}
