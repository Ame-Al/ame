...
description: Create a Snowflake programmatic access token and connect it to Ame so workflows can query your account
---
...
Ame stores the token alongside your account host as one credential. Once it is added, every Snowflake block picks it from a dropdown — and the block's database, schema, table, warehouse, role, file-format, and procedure fields become pickers that list what the token can actually see.
...
If your account has no network policy, either create one (allowing Ame's egress) or set `NETWORK_POLICY_EVALUATION = ENFORCED_NOT_REQUIRED` on an authentication policy applied to the user.
</Callout>
...
A token's expiry is fixed at creation. To rotate, generate a new token on the same user and update the credential in Ame — the old one stays valid until you remove it. `ALTER USER ... REMOVE PROGRAMMATIC ACCESS TOKEN <name>` revokes immediately and cannot be undone.
