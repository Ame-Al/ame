...
description: Set up a monday.com personal API token on a dedicated integration user to use monday.com in Ame workflows
---
...
<Steps>
  ...
  <Step>
    Search for "Monday" and open it, then click **Add to Ame** and choose **Add API token**
  ...
  <Step>
    Click **Add API token**. Ame verifies the token by querying monday.com's `me` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "Why do you recommend a dedicated integration user?", answer: "The token is bound to a user, and monday.com has no org-level token. If the token comes from an employee's personal account, it dies when that person is deactivated during offboarding. A dedicated seat like ame-bot@yourcompany.com survives team changes and makes the bot's activity auditable." },
  { question: "How do I rotate the token?", answer: "Carefully. monday.com allows only one token per user, and regenerating immediately kills the old one — there's no dual-token window. Regenerate in monday.com, then update the credential in Ame right away. Expect a brief outage for any workflows that run in between." },
