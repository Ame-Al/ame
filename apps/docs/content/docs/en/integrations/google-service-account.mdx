...
Before adding a service account to Ame, you need to configure it in the Google Cloud Console and Google Workspace Admin Console.
...
The table below lists every Google service that supports service account authentication in Ame, the API to enable in Google Cloud Console, and the delegation scopes to authorize. Copy the scope string for each service you need and paste it into the Google Workspace Admin Console.
...
Once Google Cloud and Workspace are configured, add the service account as a credential in Ame.
...
<Steps>
  ...
  <Step>
    Search for "Google Drive" and open it — any Google integration works, since they share one service account — then click **Add to Ame** and choose **Add service account**
...
<FAQ items={[
  ...
  { question: "How do I rotate the service account key?", answer: "Create a new JSON key in the Google Cloud Console under your service account's Keys tab, then update the credential in Ame with the new key. Delete the old key from Google Cloud once the new one is working." },
