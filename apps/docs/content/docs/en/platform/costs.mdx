...
Ame automatically calculates the cost of every workflow run from AI model usage plus a base run charge.

## Credits

Ame uses **credits** as the unit of measurement for all usage. **1 credit = $0.005**.

All plan limits, usage meters, and billing thresholds are displayed in credits throughout the Ame UI. Dollar amounts in this documentation are provided for reference.
...
<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** - Ame provides API keys with a 1.1x pricing multiplier for Agent blocks:
...
When workflows use tool blocks with Ame's hosted API keys, costs are charged per operation. Use your own keys via BYOK to pay providers directly instead.
...
Use your own API keys for supported providers instead of Ame's hosted keys to pay base prices with no markup.
...
|-------|------------|----------------|------|
| **Workspace** | That workspace only | Workspace **admin** | Any plan on Ame Cloud |
| **Organization** | Every current and future workspace in the organization | Organization **admin** or **owner** | Any organization plan on Ame Cloud — Pro for Teams, Max for Teams, or Enterprise |
...
3. Otherwise, Ame's hosted key, with the multiplier applied
...
Deleting a workspace key makes that workspace fall back to the organization key if one exists, and to Ame's hosted keys otherwise. Deleting an organization key makes every workspace that was inheriting it fall back the same way.

If your organization's plan lapses, organization keys stop applying and those workspaces fall back to Ame's hosted keys with the multiplier. The keys are retained, and organization admins can still delete them, but adding or updating them requires an active organization plan.
...
Ame has two paid plan tiers - **Pro** and **Max**. Either can be used individually or with a team. Team plans pool credits across all seats in the organization.
...
Ame uses a **base subscription + overage** billing model:
...
When on-demand is enabled and unbilled overage reaches $100, Ame automatically bills the full unbilled amount.
...
<FAQ items={[
  ...
  { question: "What is the credit-to-dollar conversion rate?", answer: "1 credit equals $0.005. All plan limits, usage meters, and billing thresholds in the Ame UI are displayed in credits." },
  ...
  { question: "How does the 1.1x hosted model multiplier work?", answer: "When you use Ame's hosted API keys (instead of bringing your own), a 1.1x multiplier is applied to the base model pricing for Agent blocks. This covers infrastructure and API management costs. You can avoid this multiplier by using your own API keys via the BYOK feature." },
  ...
  { question: "When does threshold billing trigger?", answer: "When on-demand billing is enabled and your unbilled overage reaches $100, Ame automatically bills the full unbilled amount. This spreads large charges throughout the month instead of accumulating one large bill at period end." },
]} />
