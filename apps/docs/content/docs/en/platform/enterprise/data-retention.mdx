...
<FAQ items={[
  {
    ...
    answer: "Only organization owners and admins can configure data retention settings. On Ame Cloud, the organization must be on an Enterprise plan."
  },
  {
...
Once enabled, retention settings are configurable through **Settings → Enterprise → Data Retention** the same way as Ame Cloud.

### PII redaction

PII redaction runs against a standalone [Presidio](https://microsoft.github.io/presidio/) service. Deploy it (see `apps/pii`) and point Ame at it, then enable the redaction surfaces:
