...
Ame resolves the governing group for a user in a workspace deterministically:
...
When a user runs a workflow or uses Chat, Ame reads the resolved group's configuration and applies it:
...
- **In Chat:** Disallowed blocks are filtered out of the block list so they cannot be added to a workflow. Disallowed tool types (MCP, custom tools, skills) are skipped if Ame attempts to use them.
...
<Image src="/static/enterprise/access-control-model-providers.png" alt="Model Providers tab showing a grid of AI providers including Ollama, vLLM, OpenAI, Anthropic, Google, Azure OpenAI, and others with checkboxes to allow or restrict access" width={900} height={500} /> The list shows all providers available in Ame.
...
| API Keys | Hides the Ame Keys tab in Settings |
...
| Skills | Disables the use of Ame Skills in workflows and agents |
...
| Feature | Effect when checked |
|---------|-------------------|
| Ame Mailer | Hides the Ame Mailer (Inbox) feature |
...
- If Ame generates a workflow step that would use a disallowed tool (MCP, custom, or skills), that step is skipped and the reason is noted.
...
<FAQ items={[
  ...
  },
  {
    ...
    answer: "Yes. Ame reads the user's permission group for the workspace's organization before suggesting blocks or tools. Disallowed blocks are filtered out of the block picker, and disallowed tool types are skipped during workflow generation."
  },
  {
...
Once enabled, permission groups are managed through **Settings → Enterprise → Access Control** the same way as Ame Cloud.
