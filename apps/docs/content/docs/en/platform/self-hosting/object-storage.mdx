...
description: Configure where Ame stores uploaded files — local disk, AWS S3, Azure Blob, or Google Cloud Storage
---
...
Ame stores every uploaded file — knowledge base documents, chat attachments, execution outputs, profile pictures, and more — in object storage. Four backends are supported:
...
Set `STORAGE_PROVIDER` to `local`, `s3`, `azure`, or `gcs` to select a backend explicitly. When it is unset, Ame infers the backend from the configured environment variables in this order:
...
If `STORAGE_PROVIDER` is unset, Ame skips incomplete backends and uses the first ready match in that order. A higher-priority backend with its required fields present but invalid supplied values fails fast instead of silently falling through. An explicit `STORAGE_PROVIDER` takes precedence and must be valid and complete.
...
Ame separates files into purpose-specific buckets. **Ame never creates buckets** — create each one yourself before configuring it. Only `S3_OG_IMAGES_BUCKET_NAME` and `S3_WORKSPACE_LOGOS_BUCKET_NAME` fall back to the general bucket; the others resolve to their own literal defaults, so set every bucket you intend to use.
...
Keep all buckets **private** (block public access). Ame serves files through short-lived presigned URLs, so the buckets never need public read access.
...
Uploads are sent **directly from the browser** to S3 via presigned `PUT` requests, so each bucket needs a CORS policy that allows your Ame origin. Without this, every upload fails with a CORS error in the browser console even though the server-side configuration is correct.
...
Set `AllowedOrigins` to your exact Ame origin (scheme + host, no trailing slash). Add every origin users reach Ame from, including an apex/`www` pair if both are live.
...
Create an IAM policy scoped to your buckets and attach it to the user (or role) Ame runs as:
...
- **Instance/role credentials (recommended)** — attach the policy to the EC2 instance role, ECS task role, or EKS IRSA role. Leave `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` unset and Ame falls back to the default AWS credential chain automatically.
...
Only `AWS_REGION` and `S3_BUCKET_NAME` are strictly required to switch Ame into S3 mode. Add the others so each file type lands in its own bucket.
...
Ame origin, `GET` and `PUT`, the `Content-Type` header, and the `x-ms-*` prefix used by signed blob
...
Keep all buckets **private** (no `allUsers` bindings). Ame serves files through short-lived V4 signed URLs, so the buckets never need public read access.

Because uploads are sent **directly from the browser** via signed `PUT` requests, each bucket needs a CORS policy that allows your Ame origin:
...
Only `GCS_BUCKET_NAME` is strictly required to switch Ame into GCS mode. Every purpose-specific bucket falls back to the general bucket when unset — add the others so each file type lands in its own bucket.
...
<Callout type="warning">
  **The endpoint must be reachable from your users' browsers, and the bucket needs CORS.** Uploads use presigned `PUT` requests sent **directly from the browser** to `S3_ENDPOINT` (downloads are proxied back through the app, so they only need server-side reachability). This means:
  ...
  - Configure a **CORS policy** on the bucket that allows your Ame origin (`PUT`, `GET`, and the `Authorization` / `Content-Type` / `x-amz-*` headers). This applies to AWS S3 too — R2 and MinIO are no different.
</Callout>
...
`http://` works server-side, but since the browser uploads directly to this endpoint, prefer a TLS endpoint your users can reach (a mixed-content `http://` target will be blocked on an `https://` Ame origin).
...
Ame uploads directly to a create-only final object key and keeps upload-session state in PostgreSQL.
