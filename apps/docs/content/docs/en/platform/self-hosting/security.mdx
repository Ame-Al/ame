...
Cloud metadata endpoints stay blocked either way. This defaults to `false` because Ame's chart is deliberately stricter than the common chart default, which permits unrestricted egress.
...
Workflows can execute user-authored JavaScript and Python. Know which sandbox you are running before you expose Ame to untrusted authors.
...
<Callout type="warn">
  With the default in-process sandbox, treat everyone who can author a workflow as someone running code in your app container's security context. If your Ame instance is open to a wide or partly-trusted audience, use a remote sandbox provider and enable the NetworkPolicy egress restrictions.
</Callout>
...
Ame blocks outbound requests from database and connector tools to private, reserved, and loopback addresses. This stops a workflow from being used to scan your internal network.
...
- [ ] Object storage buckets private, with CORS limited to your Ame origin
