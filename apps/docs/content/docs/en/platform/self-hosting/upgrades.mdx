...
Ame publishes images to GHCR, tagged by release alongside `latest`:
...
`npx ame-setup update` pulls the versions configured by `SIM_VERSION` (or `latest` when it is
...
Do not manually edit the migrations table to skip a failed migration — the schema and Ame's expectations will diverge in ways that surface much later.
