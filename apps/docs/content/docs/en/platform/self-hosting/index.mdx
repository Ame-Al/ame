---
title: Self-Hosting
description: Deploy Ame on your own infrastructure
---
...
Deploy Ame on your own infrastructure with Docker or Kubernetes.
...
Both deploy the same feature set. Docker Compose is the fastest way to evaluate Ame and is fine for a single-node team install; Kubernetes is the path for high availability and managed secrets.
...
Ame is self-contained for the core editor and execution engine. A few features reach outside the deployment:
...
| **Chat module** | `COPILOT_API_KEY` from ame.ai | Set `NEXT_PUBLIC_CHAT_DISABLED=true` to hide the module instead. |
...
<FAQ items={[
  { question: "What are the minimum requirements to self-host Ame?", answer: "At minimum you need 2 CPU cores, 12 GB RAM, 20 GB SSD storage, and Docker 20.10 or later. Memory is typically the constraining factor due to workflow execution (isolated-vm sandboxing), file processing, and vector operations (pgvector)."},
  ...
  { question: "Can I use Ame with local AI models?", answer: "Yes. Ame supports Ollama for local model inference. Use docker-compose.ollama.yml instead of docker-compose.prod.yml. It offers both GPU (with NVIDIA support) and CPU-only profiles, and automatically pulls gemma3:4b as a starter model." },
]} />
