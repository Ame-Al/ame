...
Click **+ Add to Ame** to open the skill create page, which takes three fields:
...
Tagging a skill loads its full instructions into the conversation, so Ame follows them for that request — no waiting for Ame to decide the skill is relevant on its own.
...
<FAQ items={[
  ...
  { question: "Do skills work with all LLM providers?", answer: "Yes. The load_skill mechanism uses standard tool-calling, which is supported by all LLM providers in Ame. No provider-specific configuration is needed. The skill system works the same way whether you are using Anthropic, OpenAI, Google, or any other supported provider." },
  ...
  { question: "Can I use skills in Chat?", answer: "Yes. Type / in the Chat message box to open the skills menu and pick a skill — for example /format-markdown. The tag loads the skill's full instructions into the conversation, so Ame follows them for that request without having to decide on its own to load the skill." },
]} />
