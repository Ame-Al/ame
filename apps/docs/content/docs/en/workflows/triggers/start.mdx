...
<Tabs items={['Editor run', 'API', 'Chat']}>
  ...
  <Tab>
    The Input Format becomes the JSON request body. Each field is part of the body, and Ame coerces primitive types on the way in. File fields expect objects that reference uploaded files, so upload first, then invoke the workflow.
  </Tab>
  <Tab>
