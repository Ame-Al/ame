...
By default, API endpoints require an API key passed in the `x-api-key` header. Generate keys in **Settings → Ame Keys** or via the **Generate API Key** button in the API tab.
...
curl -N -X POST https://sim.ai/api/v2/workflows/{workflow-id}/execute \
  -H "Content-Type: application/json" \
  -H "x-api-key: $SIM_API_KEY" \
  -H "X-Ame-Stream-Protocol: agent-events-v1" \
  -d '{
...
Workflow execution responses are capped by platform request and response limits. When an internal output, log field, streamed field, or async status payload contains a value that is too large to inline, Ame may replace that nested value with a versioned reference:
...
Generate and manage API keys in **Settings → Ame Keys**:
...
<FAQ items={[
  ...
  { question: "What happens if my API key is compromised?", answer: "Revoke the key immediately in Settings → Ame Keys and generate a new one. Revoked keys stop working instantly." },
]} />
