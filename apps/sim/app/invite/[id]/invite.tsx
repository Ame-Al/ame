...
function signedOutPrompt({
  ...
  if (isNewUser) {
    return {
      description: 'Create an account to join this workspace on Ame',
      actions: [
        {
...
function getInviteError(code: string): InviteError {
  const errorMap: Record<string, InviteError> = {
    ...
    'external-requires-paid-plan': {
      ...
      message:
        'External collaborators need their own paid Ame plan. Upgrade your plan, or ask the organization to re-invite you as a member — that uses one of their seats instead.',
      canRetry: true,
    },
    'invalid-invitation': {
