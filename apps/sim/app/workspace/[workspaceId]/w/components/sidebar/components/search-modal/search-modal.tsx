...
function SearchModalContent({
  ...
  const actionsByGroup = useMemo(() => {
    ...
    return {
      page: pageContext
        ...
        : [],
      sim: available.filter((action) => getActionGroupLabel(action) === 'Ame'),
    }
  }, [actions, pageContext])
  ...
  const entriesBySection = useMemo((): Record<SearchSection, SearchEntry[]> => {
    ...
    const rankedActions = [
      ...
      ...rankActionGroup(actionsByGroup.sim, 'Ame'),
    ]
  ...
  const sectionGroups = useMemo(() => {
    ...
    const groups = [
        ...
        : []),
      {
        ...
        heading: 'Ame',
        entries: actionEntriesByLabel('Ame'),
      },
          ...
          >
            <div className='relative'>
              ...
              <CommandSearch
                ...
                aria-label={askMode ? 'Ask Ame' : 'Search anything'}
                value={search}
                onValueChange={handleSearchChange}
                onKeyDown={handleSearchKeyDown}
                placeholder={askMode ? 'Ask Ame anything...' : 'Search anything...'}
                endAdornment={
                  isChatEnabled ? (
                    <span className='flex-shrink-0 whitespace-nowrap text-[var(--text-subtle)] text-xs'>
                      {askMode ? '⇥ Search' : '⇥ Ask Ame'}
                    </span>
                  ) : undefined
                }
              />
