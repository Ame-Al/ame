...
export const LOGS_PAGE_DESCRIPTION =
  'Ame gives teams AI agent observability: trace every run step by step, search and filter across runs, and catch failures with alerts.'
...
const LOGS_CONFIG: SolutionsPageConfig = {
  ...
  hero: {
    eyebrow: 'Logs',
    heading: 'Trace every agent run with full observability in Ame.',
    description:
      'Logs is the AI agent observability layer in Ame, the open-source AI workspace. Follow each run block by block, search across every run, and catch failures with alerts.',
    summary:
      'Logs is the AI agent observability layer in Ame, the open-source AI workspace where teams build, deploy, and manage AI agents. Ame records every step of every run, so teams trace each decision, tool call, and output, and catch failures with alerts.',
    visual: (
      <PlatformHeroVisual>
        <LogsHeroLoop />
      </PlatformHeroVisual>
  ...
  rows: [
    {
      ...
      subtitle:
        'Ame records every step of every agent run, so teams can follow each decision, tool call, and output on one timeline.',
      cta: { label: 'Explore Logs in Ame', href: '/signup' },
      cards: [
        {
          title: 'Trace runs block by block',
          description:
            'Ame captures every step of a run: each block, tool call, and model response, with its duration.',
        ...
        },
        {
          ...
          description:
            'Ame shows each run the moment it starts, with its trigger, duration, and output, so teams see agents working in real time.',
          visual: (
            <RunMonitoringGraphic
              fields={[
        ...
        },
        {
          ...
          description:
            'Ame keeps a permanent history of every run and follow-up, so teams can answer what happened, when, and who resolved it.',
          visual: (
            <AuditTrailGraphic
              entries={[
                {
    ...
    },
    {
      ...
      subtitle:
        'Ame surfaces failed runs the moment they happen, with the alerts and context teams need to fix them fast.',
      cta: { label: 'Start tracing runs', href: '/signup' },
      cards: [
        {
          title: 'Filter to the runs that matter',
          description:
            'Search and filter every run in Ame by workflow, status, trigger, and time, so failed runs surface fast.',
          visual: <FilterRunsGraphic />,
        },
        {
          title: 'Debug with full context',
          description:
            'Ame snapshots each run, so teams open a failed run with inputs, outputs, and trace intact and see which block broke.',
          visual: (
            <StagingGraphic
        ...
        },
        {
          ...
          description:
            'When a run fails, Ame alerts the channel your team already watches, naming the failing block and linking straight to the trace.',
          featureTileTone: 'dark',
          featureTileDescriptionTone: 'soft',
