...
export const TABLES_PAGE_DESCRIPTION =
  'Tables is the AI agent database built into Ame. Store leads, tickets, and invoices as rows agents read and write, and carry state from one run to the next.'
...
const TABLES_CONFIG: SolutionsPageConfig = {
  ...
  hero: {
    eyebrow: 'Tables',
    heading: 'Give AI agents a database built into Ame.',
    description:
      'Tables is the database built into Ame, the open-source AI workspace. Store the records agents read and write, fill empty cells with enrichments, and keep state between runs.',
    summary:
      'Tables is the AI agent database built into Ame, the open-source AI workspace where teams build, deploy, and manage AI agents. Teams store records agents read and write, run enrichments that fill cells automatically, query rows from agent logic, and keep state between runs, all in one workspace.',
    visual: (
      <PlatformHeroVisual>
        <TablesHeroLoop />
      </PlatformHeroVisual>
  ...
  rows: [
    {
      ...
      subtitle:
        'Ame stores the leads, tickets, and invoices agents work with as tables in the same workspace as the agents.',
      cta: { label: 'Explore Tables', href: '/signup' },
      cards: [
        {
          title: 'Records agents act on',
          description:
            'Store leads, tickets, and invoices as rows agents read and write. Ame keeps the data next to the agents that work it.',
          visual: <TableGridGraphic />,
        },
        {
          title: 'Enrichments fill the blanks',
          description:
            'Ame runs enrichments over new rows, finding work emails, phone numbers, and company info, so empty cells fill themselves.',
          visual: <EnrichmentFillGraphic />,
        },
        {
          title: 'Query from agent logic',
          description:
            'Read and write rows from code or any workflow block. Ame treats every table as part of your agent logic.',
    ...
    },
    {
      ...
      subtitle:
        'Ame carries state between runs. Every run writes rows, every change is recorded, and agents pick up exactly where they left off.',
      cta: { label: 'See how agents use Tables', href: '/signup' },
      cards: [
        {
          title: 'State between runs',
          description:
            'Each run writes its results back to the table, so the next run starts from what Ame already knows.',
          visual: (
            <RunMonitoringGraphic
              fields={[
        ...
        },
        {
          ...
          description:
            'Ame routes records from your tools into tables and back out again, one switchboard for structured data.',
          featureTileTone: 'dark',
          featureTileDescriptionTone: 'soft',
          visual: (
            <OperationsTeamsGraphic
        ...
        },
        {
          ...
          description:
            'Ame records every insert and update with who made it, so the history of a table is always inspectable.',
          visual: (
            <AuditTrailGraphic
              entries={[
                {
