...
function buildModelSummary(
  ...
  const parts = [
    `${displayName} is a ${providerName} model tracked in Ame.`,
...
export function buildProviderFaqs(provider: CatalogProvider): CatalogFaq[] {
  ...
  const faqs: CatalogFaq[] = [
    {
      question: `What ${provider.name} models are available in Ame?`,
      answer: `Ame currently tracks ${provider.modelCount} ${provider.name} model${provider.modelCount === 1 ? '' : 's'} including ${provider.models
        .slice(0, 6)
        .map((model) => model.displayName)
        .join(', ')}${provider.modelCount > 6 ? ', and more' : ''}.`,
    },
    {
      question: `What is the default ${provider.name} model in Ame?`,
      answer: provider.defaultModel
        ? `${provider.defaultModelDisplayName} is the default ${provider.name} model in Ame.`
        : `${provider.name} does not have a fixed default model in the public catalog because models are loaded dynamically.`,
    },
    {
      question: `What is the cheapest ${provider.name} model tracked in Ame?`,
      answer: cheapestModel
        ...
        : `Ame does not currently expose a fixed public pricing table for ${provider.name} models on this page.`,
    },
    {
  ...
  if (toolUseModels.length > 0) {
    faqs.push({
      question: `Which ${provider.name} models support tool use and function calling in Ame?`,
      answer:
        toolUseModels.length === provider.modelCount
          ? `All ${provider.name} models in Ame support tool use and function calling, allowing agents to invoke external APIs, query databases, and run custom actions.`
          : `${toolUseModels
              .slice(0, 5)
              .map((m) => m.displayName)
              .join(
                ', '
              )}${toolUseModels.length > 5 ? ', and others' : ''} support tool use and function calling in Ame, enabling agents to invoke external APIs and run custom actions.`,
    })
  }
...
export function buildModelFaqs(provider: CatalogProvider, model: CatalogModel): CatalogFaq[] {
  const faqs: CatalogFaq[] = [
    {
      ...
      answer: `${model.displayName} is a ${provider.name} model available in Ame. ${model.summary}`,
    },
    {
    ...
    },
    {
      ...
      answer: model.contextWindow
        ? `${model.displayName} supports a context window of ${formatTokenCount(model.contextWindow)} tokens in Ame. In an agent, this determines how much conversation history, tool outputs, and retrieved documents the model can hold in a single call.`
    ...
    },
    {
      ...
      answer:
        model.capabilityTags.length > 0
          ? `${model.displayName} supports the following capabilities in Ame: ${model.capabilityTags.join(', ')}.`
          : `${model.displayName} supports standard text generation in Ame. No additional capability flags such as tool use or structured outputs are currently tracked for this model.`,
    },
  ]
  ...
  if (model.bestFor) {
    faqs.push({
      ...
      answer: `${model.bestFor} When used in a Ame workflow, it can be selected in any Agent block from the model picker.`,
    })
  }
