...
const faqItems = [
  {
    ...
    answer:
      'The most important factors for agent tasks are reliable tool use (function calling), a large enough context window to track conversation history and tool outputs, and consistent instruction following. In Ame, OpenAI GPT-4.1, Anthropic Claude Sonnet, and Google Gemini 2.5 Pro are popular choices, and each supports tool use, structured outputs, and context windows of 128K tokens or more. For cost-sensitive or high-throughput agents, Groq and Cerebras offer significantly faster inference at lower cost.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'Tool use, also called function calling, lets an agent invoke external APIs, query databases, run code, or take any action you define. In Ame, all first-party models from OpenAI, Anthropic, Google, Mistral, Groq, Cerebras, and xAI support tool use. Look for the Tool Use capability tag on any model card in this directory to confirm support.',
  },
  {
    question: 'How do I add a model to a Ame agent?',
    answer:
      'Open Ame, add an Agent block, and select your provider and model from the model picker inside that block. Every model listed in this directory is available in the Agent block. Swapping models takes one click and does not affect the rest of your agent, making it straightforward to test different models on the same task without rebuilding anything.',
  },
]
  ...
  return withFilteredNoindex(
    {
      ...
      description: `Compare ${TOTAL_MODELS}+ AI models across ${TOTAL_MODEL_PROVIDERS} providers in Ame's AI workspace. Compare pricing, context windows, and capabilities for your agents.`,
      keywords: [
        'AI models directory',
        'AI model comparison',
        'LLM model list',
...
export default async function ModelsPage({
  ...
  const itemListJsonLd = {
    ...
    description: `Directory of ${TOTAL_MODELS} AI models tracked in Ame across ${TOTAL_MODEL_PROVIDERS} providers.`,
    url: `${baseUrl}/models`,
