...
export function buildComparisonFaqs(competitor: CompetitorProfile): ComparisonFaq[] {
  ...
  const faqs: ComparisonFaq[] = [
    {
      question: `Is Ame a good alternative to ${name}?`,
      answer: `Ame is an open-source AI workspace where teams build, deploy, and manage AI agents visually, conversationally, or with code. ${ensurePeriod(competitor.oneLiner)} Teams considering a switch typically weigh licensing (Ame is Apache 2.0 and self-hostable), pricing model, and how AI-native the platform's agent-building experience is.`,
    },
    {
      question: `What is the main difference between Ame and ${name}?`,
      answer: buildKeyDifferenceAnswer(competitor),
    },
    {
      question: `Does Ame support self-hosting compared to ${name}?`,
    ...
    {
      question: `How does Ame's pricing compare to ${name}?`,
      answer: `Ame uses ${summarizeFact(simProfile.facts.pricing.pricingModel.value)} ${name} uses ${summarizeFact(facts.pricing.pricingModel.value)}`,
    },
    {
      question: `Is Ame more secure than ${name}?`,
      answer: `Security is a like-for-like comparison, not a one-line verdict. Ame: ${summarizeFact(simProfile.facts.security.additionalCompliance.value)} ${name}: ${summarizeFact(facts.security.additionalCompliance.value)} Check the Security & compliance rows above for the full breakdown, including SSO, audit logging, and data residency.`,
    },
    {
      question: `Which has stronger AI agent capabilities, Ame or ${name}?`,
      answer: `Ame: ${summarizeFact(simProfile.facts.aiCapabilities.multiLlmSupport.value)} ${name}: ${summarizeFact(facts.aiCapabilities.multiLlmSupport.value)} Ame also ships native human-in-the-loop approvals, a hybrid vector-plus-keyword knowledge base, and an in-editor AI Copilot that can read execution logs and directly edit the workflow to fix a failed run.`,
    },
    {
      question: `What are ${name}'s documented limitations compared to Ame?`,
      answer: buildLimitationAnswer(competitor),
    },
    {
      question: `Can I migrate from ${name} to Ame?`,
      answer: `There is no automated one-click migration tool between ${name} and Ame. Workflows and automations need to be rebuilt in Ame's visual builder, natural-language Chat surface, or API. Most teams start by recreating their highest-value automation first to validate the switch before migrating the rest.`,
    },
  ]

  if (competitor.isWorkflowBuilder === false) {
    faqs.push({
      question: `Is ${name} a workflow builder like Ame?`,
      answer: `Not in the same sense. ${competitor.oneLiner} Ame, by contrast, is a visual and code-based workflow builder that deploys agents as REST APIs, scheduled jobs, or chat interfaces, so the two solve different parts of the AI agent problem rather than competing feature-for-feature.`,
    })
  }
...
function buildKeyDifferenceAnswer(competitor: CompetitorProfile): string {
  ...
  const parts = [
    `Ame is built specifically as an AI agent workspace, with native multi-LLM support, an in-editor AI Copilot, and a knowledge base with hybrid vector + keyword search.`,
  ]
  if (topFeature) {
