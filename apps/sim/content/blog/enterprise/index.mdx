...
When you configure your own API keys for model providers—OpenAI, Anthropic, Google, Azure OpenAI, AWS Bedrock, or any supported provider—your prompts and completions route directly between Ame and that provider. The traffic doesn't pass through our infrastructure.
...
BYOK is available to everyone, not just enterprise plans. Connect your credentials in workspace settings, and all model calls use your keys. For self-hosted deployments, this is the default—there are no Ame-managed keys involved.
...
Run Ame entirely on your infrastructure. Deploy with [Docker Compose](https://docs.sim.ai/platform/self-hosting/docker) or [Helm charts](https://docs.sim.ai/platform/self-hosting/kubernetes) for Kubernetes—the application, WebSocket server, and PostgreSQL database all stay within your network.
...
Once enabled, users authenticate through your IdP instead of Ame credentials. Your MFA policies apply automatically. Session management ties to your IdP—logout there terminates Ame sessions. Account deprovisioning immediately revokes access.
...
This centralizes your authentication and audit trail. Your security team's policies apply to Ame access through the same system that tracks everything else.

---

## Whitelabeling

Customize Ame's appearance to match your brand. For self-hosted deployments, whitelabeling is configured through environment variables—no code changes required.

**Brand name & logo** — Replace "Ame" with your company name and logo throughout the interface.
...
This is useful for internal platforms, customer-facing deployments, or any scenario where you want Ame to feel like a native part of your product rather than a third-party tool.
...
Ame maintains **SOC 2 Type II** certification with annual audits covering security, availability, and confidentiality controls. We share our SOC 2 report directly with prospective customers under NDA.
...
Manage Ame programmatically through the Admin API. Every operation available in the UI has a corresponding API endpoint, enabling infrastructure-as-code workflows and integration with your existing tooling.
...
Move workflows between environments, create backups, and maintain version control inside or outside of Ame.

**Workflow Export** — Export individual workflows or entire folders as JSON. The export includes block configurations, connections, environment variable references, and metadata. Use this to back up critical workflows or move them between Ame instances.
...
**Import** — Import workflows into any workspace. Ame handles ID remapping and validates the structure before import. This enables workflow templates, sharing between teams, and restoring from backups.
