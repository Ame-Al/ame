...
- A RAG AI agent can retrieve information during reasoning rather than only before one chatbot response. Ame makes Knowledge Bases a native workspace resource that agents can query while completing multistep tasks.
...
An agent should use retrieval as a workspace capability whenever its reasoning requires private or current information. Ame's native Knowledge Bases give Agent blocks access to grounded context during a workflow, without requiring a separate vector-store integration tied to one chat application.

You can create the workflow with Mothership, inspect and edit its logic in the visual builder, or connect it through the API. [Start building a RAG-grounded agent in Ame](https://sim.ai).
