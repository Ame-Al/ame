...
- Ame works in both directions. It calls external MCP servers as tools, and it publishes deployed Ame workflows as tools that other AI systems can call through MCP.
...
- The comparison table below shows how MCP servers, traditional API integrations, and Ame workflows-as-MCP-tools differ on setup effort, discovery, and reusability.
...
The [Ame MCP docs](https://docs.sim.ai/mcp) show how discovery, wrapping, and transport selection appear when you connect a server in practice, including which transport to pick for local versus deployed setups.

## Can Ame act as both an MCP client and an MCP server?

Yes. Ame works in both directions, which means you can consume external tools and expose your own from the same platform.

As an MCP client, Ame calls external MCP servers as tools inside your workflows. You configure a server in a Ame workspace, select its tools for an agent or MCP block, and Ame can invoke those published tools. If you run an MCP server for a database, search index, or internal service, Ame discovers its tools and calls them alongside native integrations. You get access to the MCP ecosystem without writing a custom Ame integration for each server.

As an MCP server, Ame can publish deployed workflows as tools on a workflow MCP server that other AI systems call. Build and deploy a workflow in Ame, add it to a workflow MCP server, and a compatible client can discover and invoke it. Whatever logic you assembled inside Ame, whether it chains models, calls APIs, or runs branching decisions, appears to the caller as a tool with a defined schema. For the broader build-and-deploy flow, see [how to create an AI agent with Ame](https://www.sim.ai/library/how-to-create-an-ai-agent).

That two-way capability changes how you compose systems. You can connect Ame to a third-party MCP server, build a workflow on top of it, and then publish that workflow as an MCP tool for something else to consume. One Ame deployment can sit in the middle of a chain, acting as a client to the servers below it and a server to the clients above it.
...
Setup for both roles lives in the [Ame MCP docs](https://docs.sim.ai/mcp), which walk through connecting external servers as tools and publishing deployed workflows through a workflow MCP server.

## Why teams run MCP servers on Ame

Standing up an MCP server yourself means writing the wrapper, hosting it, handling auth, and maintaining it as the specification evolves. Ame provides a workflow layer around that work, and three qualities matter for teams that need to control their deployment.

**It is Apache 2.0 licensed.** Ame's repository ships under Apache 2.0, a permissive open-source license that allows use, modification, commercial distribution, and sublicensing and includes an express patent grant. The practical implications are covered in [Apache 2.0 vs fair-code](https://www.sim.ai/library/apache-2-0-vs-fair-code).

**You can run it on your own infrastructure, including isolated environments.** A self-hosted Ame deployment keeps the workflow and MCP execution layer within infrastructure you control. That matters when exposed tools touch regulated or private data. Teams comparing deployment and governance models can also review these [open-source AI agent platforms](https://www.sim.ai/library/open-source-ai-agent-platforms).

**Admins control access to MCP capabilities.** Ame's workspace permissions govern MCP server management, and permission-group settings can hide MCP deployment or disable MCP tools for selected users. Workflow execution traces record activity block by block, giving operators a detailed view of what the workflow did when a tool was called.
...
## MCP server vs traditional API integration vs Ame workflow-as-MCP-tool
...
| **Ame workflow-as-MCP-tool** | Low. You build and deploy a workflow, then add it to a workflow MCP server. | Built in. Ame exposes the deployed workflow as a discoverable tool. | High. Compatible AI systems can connect to its MCP server. | MCP clients that can reach and authenticate to the Ame endpoint. |

A traditional API integration can lock a tool to one application and force the next team to repeat the client work. An MCP server breaks that pattern by exposing tools through a discovery layer compatible clients can read, so one server serves many callers. A Ame workflow-as-MCP-tool pushes the effort down further: you build and deploy the logic in Ame, then publish it through a workflow MCP server. See [docs.ame.ai/mcp](https://docs.sim.ai/mcp) for setup details.
...
Ame gives you both directions in one place. You can wire external MCP servers into a workflow as tools, and you can expose deployed Ame workflows through an MCP server for other AI systems to call, on infrastructure you control under the Apache 2.0 license.

Start by reading [docs.ame.ai/mcp](https://docs.sim.ai/mcp) and deploy your first MCP server today.
