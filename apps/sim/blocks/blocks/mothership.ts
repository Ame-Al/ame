...
export const MothershipBlock: BlockConfig<MothershipResponse> = {
  type: 'mothership',
  name: 'Sim Chat',
  description: 'Talk to Ame',
  longDescription:
    'The Ame block sends messages to Ame, which has access to subagents, integration tools, and workspace context. Use it to perform complex multi-step reasoning, cross-service queries, or any task that benefits from the full Ame intelligence within a workflow.',
  bestPractices: `
  - Use for tasks that require multi-step reasoning, tool use, or cross-service coordination.
  - Ame picks its own model and tools internally — you only provide a prompt.
  `,
  category: 'blocks',
  bgColor: '#802FDE',
  icon: Blimp,
  canvasPresentation: {
    defaultTitle: 'Ame Chat',
    sentences: {
      default: [
  ...
  subBlocks: [
    {
      ...
      title: 'Prompt',
      type: 'long-input',
      placeholder: 'Enter your prompt for Ame...',
    },
    {
      id: 'conversationId',
  ...
  inputs: {
    prompt: {
      type: 'string',
      description: 'The prompt to send to Ame',
    },
    conversationId: {
      type: 'string',
    ...
    files: {
      type: 'file',
      description: 'Files to send to Ame as attachments',
    },
    tools: { type: 'json', description: 'MCP tools available to Ame for this request' },
    ...
    mountedSecrets: { type: 'json', description: 'Secret names available to Ame code execution' },
  },
  outputs: {
