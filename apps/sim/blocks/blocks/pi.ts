...
export const PiBlock: BlockConfig<PiResponse> = {
  ...
  - Use Local Dev to edit a repo on your own machine; expose the machine on a public hostname/tunnel so Ame can reach it over SSH.
  - Plan, Create PR, and Update PR require your own provider API key for every model, including ones Ame hosts, because the model runs in the sandbox. Review Code and Local Dev keep the model key in Ame and can use either BYOK or a hosted key.
  ...
  subBlocks: [
...

    {
      ...
      tooltip:
        'Gives the agent a single web_search tool backed by the selected provider. Search always uses your own key for that provider, never a Ame-hosted one, because sandbox modes place the key inside the coding sandbox.',
    },
    {
      id: 'searchApiKey',
  ...
  inputs: {
    ...
    tools: { type: 'json', description: 'Ame tools exposed to the agent (Local Dev)' },
