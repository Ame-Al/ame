...
export function checkForUpdatesInteractive(
  ...
  const settle = (state: DesktopUpdateState) => {
    switch (state.status) {
      case 'available': {
        const label = state.version ? `Ame ${state.version} is available` : 'An update is available'
        void showDialog({
          type: 'info',
      ...
      case 'downloading':
        void showDialog({
          type: 'info',
          message: state.version ? `Downloading Ame ${state.version}…` : 'Downloading update…',
      ...
      default:
        void showDialog({
          ...
          detail: `Ame ${app.getVersion()} is currently the newest version available.`,
        })
    }
  }
