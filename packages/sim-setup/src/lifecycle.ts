...
function start(install: Install): void {
  if (install.kind === 'compose') {
    ...
    p.note(
      [
        `open ${APP_SIGNUP_URL}`,
        'follow logs:  npx ame-setup logs',
        'stop:         npx ame-setup stop',
      ].join('\n'),
      'Running'
    )
    return
  ...
  if (install.kind === 'dev') {
    ...
    p.note(
      ['start the dev server:  bun run dev:full', 'stop DB/Redis:         npx ame-setup stop'].join(
        '\n'
      ),
      'Ready'
    )
...
function stop(install: Install): void {
  if (install.kind === 'compose') {
    ...
    p.note(
      ['start again:  npx ame-setup start', 'remove:       npx ame-setup down'].join('\n'),
      'Stopped'
    )
    return
  }
...
function update(install: Install): void {
  ...
  p.note(
    [
      ...
      `check:   ${theme.command('npx ame-setup status')}`,
      `logs:    ${theme.command('npx ame-setup logs')}`,
    ].join('\n'),
    'Update complete'
  )
}
...
async function reset(install: Install | null): Promise<void> {
  ...
  p.note(`start fresh with ${theme.command('npx ame-setup')}`, 'Reset complete')
}
...
async function status(): Promise<void> {
  ...
  if (installs.length === 0) {
    console.log(
      docker
        ? ` ${glyph.warn} No Ame install detected — run ${theme.command('npx ame-setup')}.`
...
export async function runLifecycle(command: LifecycleCommand): Promise<void> {
  ...
  if (!install) {
    p.log.warn(`No Ame install detected. Run ${theme.command('npx ame-setup')} first.`)
    return
  }
  switch (command) {
    case 'start':
