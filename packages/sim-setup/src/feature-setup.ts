...
async function setupIntegration(
  ...
  if (!requestedId) {
    throw new Error('Missing integration id. Example: npx ame-setup add integration slack')
  }
...
export function resolveFeatureSetupDestination(
  ...
  if (sources.length === 0) {
    throw new Error('No Ame configuration was detected. Run npx ame-setup first.')
  }

  const managed = sources.filter((source) => source.managedByCurrentCheckout)
  if (managed.length === 0) {
    throw new Error(
      'No effective configuration is safely writable by this checkout. Process overrides, higher-precedence development env files, external Compose projects, and Helm releases must be updated at their source. Run npx ame-setup config for the detected sources.'
    )
  }
  if (managed.length > 1) {
    throw new Error(
      `More than one effective configuration is writable by this checkout (${managed.map((source) => source.label).join(', ')}). Run npx ame-setup config and remove the ambiguity before configuring a feature.`
    )
  }
  ...
  if (!source.values) {
    throw new Error(
      `${source.label} is managed by this checkout, but its effective environment could not be resolved. Run npx ame-setup config and fix the reported source error first.`
    )
  }
  if (source.kind === 'helm') {
    throw new Error(
      'Helm configuration cannot be updated by npx ame-setup add. Update the release Secret or values and upgrade the release.'
    )
  }
